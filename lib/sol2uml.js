...
WARNING: sol2uml does not use the Solidity compiler so may differ with solc. A known example is fixed-sized arrays declared with an expression will fail to be sized.`)
    ...
    .option('-a, --array <number>', 'Number of slots to display at the start and end of arrays.', '3')
    .action(async (fileFolderAddress, options, command) => {
    try {
        const combinedOptions = {
            ...command.parent._optionValues,
        ...
        if (combinedOptions.data) {
            ...
            await (0, converterClasses2Storage_1.addDynamicVariables)(storageSections[0], storageSections, combinedOptions.url, storageAddress, arrayItems, block);
        }
