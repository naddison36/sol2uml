...
const parseVariables = (umlClass, umlClasses, variables, storageSections, inheritedContracts, mapping) => {
    ...
    umlClass.attributes.forEach((attribute) => {
        ...
        const getValue = calcGetValue(attribute.attributeType, dynamic, mapping, referenceStorageSection?.type);
...
const parseStorageSectionFromAttribute = (attribute, umlClass, otherClasses, storageSections, mapping) => {
    if (attribute.attributeType === umlClass_1.AttributeType.Array) {
        ...
        const getValue = calcGetValue(baseAttribute.attributeType, dynamicBase, mapping, referenceStorageSection?.type);
        const variables = [];
        variables[0] = {
            id: variableId++,
            fromSlot: 0,
 ...
 */
const calcGetValue = (attributeType, dynamic, mapping, storageSectionType) => mapping === false &&
    (attributeType === umlClass_1.AttributeType.Elementary ||
        (attributeType === umlClass_1.AttributeType.UserDefined &&
