...
exports.addDynamicArrayVariables = exports.findDimensionLength = exports.calcSectionOffset = exports.isElementary = exports.calcStorageByteSize = exports.parseReferenceStorageSection = exports.convertClasses2StorageSections = exports.addStorageValues = exports.StorageSectionType = void 0;
 ...
 */
const addStorageValues = async (url, contractAddress, storageSection, blockTag) => {
    ...
    let slots = uniqueValueFromSlot.map((fromSlot) => {
        if (storageSection.offset) {
            return ethers_1.BigNumber.from(storageSection.offset).add(fromSlot);
        }
        return ethers_1.BigNumber.from(fromSlot);
    });
...
const parseVariables = (umlClass, umlClasses, variables, storageSections, inheritedContracts) => {
    ...
    umlClass.attributes.forEach((attribute) => {
        ...
        const referenceStorageSection = (0, exports.parseReferenceStorageSection)(attribute, umlClass, umlClasses, storageSections);
...
const processReferenceStorageSections = (storageSection, slotOffset, storageSections) => {
    ...
    storageSection.variables.forEach((variable) => {
        ...
        if (referenceStorageSection) {
            ...
            if (!variable.dynamic) {
                processReferenceStorageSections(referenceStorageSection, variable.fromSlot, storageSections);
            }
                ...
                processReferenceStorageSections(referenceStorageSection, 0, storageSections);
            }
        }
    });
};
const parseReferenceStorageSection = (attribute, umlClass, otherClasses, storageSections) => {
    if (attribute.attributeType === umlClass_1.AttributeType.Array) {
        ...
        if (baseAttributeType !== umlClass_1.AttributeType.Elementary) {
            ...
            referenceStorageSection = (0, exports.parseReferenceStorageSection)(baseAttribute, umlClass, otherClasses, storageSections);
        }
    ...
    if (attribute.attributeType === umlClass_1.AttributeType.UserDefined) {
        ...
        if (dependentClass.stereotype === umlClass_1.ClassStereotype.Struct) {
            const variables = parseVariables(dependentClass, otherClasses, [], storageSections, []);
            const newStorageSection = {
                id: storageId++,
                name: attribute.type,
    ...
    if (attribute.attributeType === umlClass_1.AttributeType.Mapping) {
        ...
        if (result !== null && result[1] && !(0, exports.isElementary)(result[1])) {
            ...
            if (typeClass.stereotype === umlClass_1.ClassStereotype.Struct) {
                const variables = parseVariables(typeClass, otherClasses, [], storageSections, []);
                const newStorageSection = {
                    id: storageId++,
                    name: typeClass.name,
                    type: StorageSectionType.Struct,
...
exports.parseReferenceStorageSection = parseReferenceStorageSection;
...
const addDynamicArrayVariables = (storageSection, storageSections) => {
    storageSection.variables.forEach((variable) => {
        ...
        (0, exports.addDynamicArrayVariables)(referenceStorageSection, storageSections);
        if (!variable.dynamic)
            return;
        ...
        const arraySlotSize = arrayItemSize > 16
            ? 32 * Math.ceil(arrayItemSize / 32)
            : arrayItemSize;
        const arrayLength = ethers_1.BigNumber.from(variable.value).toNumber();
        for (let i = 1; i < arrayLength; i++) {
...
exports.addDynamicArrayVariables = addDynamicArrayVariables;
