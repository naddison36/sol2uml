...
WARNING: sol2uml does not use the Solidity compiler so may differ with solc. A known example is fixed-sized arrays declared with an expression will fail to be sized.`)
    ...
    try {
        ...
        const storageSections = (0, converterClasses2Storage_1.convertClasses2StorageSections)(contractName, umlClasses, combinedOptions.contractFile);
        ...
        if (combinedOptions.data) {
            ...
            for (const storageSection of storageSections) {
                await (0, slotValues_1.addSlotValues)(combinedOptions.url, storageAddress, storageSection, combinedOptions.block);
            }
            // Add storage variables for dynamic arrays, strings and bytes
            await (0, converterClasses2Storage_1.addDynamicVariables)(storageSections[0], storageSections, combinedOptions.url, storageAddress, combinedOptions.block);
        }
