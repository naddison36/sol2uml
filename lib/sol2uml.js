...
If an Ethereum address with a 0x prefix is passed, the verified source code from Etherscan will be used. For example
    ...
    try {
        ...
        if (options.squash && !options.baseContractNames) {
            throw Error('Must specify base contract(s) when using the squash option.');
        }
        ...
        if (options.baseContractNames) {
            ...
            // squash contracts
            if (options.squash) {
                filteredUmlClasses = (0, squashClasses_1.squashUmlClasses)(filteredUmlClasses, baseContractNames);
            }
        }
