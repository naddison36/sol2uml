 ...
 */
function convertAST2UmlClasses(node, relativePath, remappings, filesystem = false, remappingsBase) {
    const imports = [];
    umlClasses = [];
                ...
                if (filesystem) {
                    ...
                    try {
                        ...
                        if (remappings.length > 0 && remappingsBase) {
                            const remappedPath = (0, exports.renameFile)(childNode.path, remappings);
                            if (remappedPath !== childNode.path) {
                                const absoluteRemappedPath = path.resolve(remappingsBase, remappedPath);
                                if ((0, fs_1.existsSync)(absoluteRemappedPath)) {
                                    const newImport = {
                                    ...
                                    imports.push(newImport);
                                    resolved = true;
                                    debug(`Resolved import via remapping: ${childNode.path} → ${absoluteRemappedPath}`);
                                }
                            }
                        }
                        if (!resolved) {
...
function parseExpression(expression, umlClass) {
    ...
    if (expression.type === 'BinaryOperation') {
        ...
        if (expression.expression.type === 'MemberAccess' &&
            ...
            const storageAttr = umlClass.attributes.find((attr) => attr.name === identifierName &&
                attr.attributeType === umlClass_1.AttributeType.UserDefined);
            if (storageAttr) {
