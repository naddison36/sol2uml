...
const convertClasses2StorageSections = (contractName, umlClasses, contractFilename) => {
    ...
    const variables = parseVariables(umlClass, umlClasses, [], storageSections, [], false);
 ...
 */
const parseVariables = (umlClass, umlClasses, variables, storageSections, inheritedContracts, mapping) => {
    ...
    newInheritedContracts.forEach((parent) => {
        ...
        parseVariables(parentClass, umlClasses, variables, storageSections, inheritedContracts, mapping);
    });
    ...
    umlClass.attributes.forEach((attribute) => {
        ...
        const referenceStorageSection = (0, exports.parseStorageSectionFromAttribute)(attribute, umlClass, umlClasses, storageSections, mapping || attribute.attributeType === umlClass_1.AttributeType.Mapping);
        // should this new variable get the slot value
        const displayValue = calcDisplayValue(attribute.attributeType, dynamic, mapping, referenceStorageSection?.type);
 ...
 */
const parseStorageSectionFromAttribute = (attribute, umlClass, otherClasses, storageSections, mapping) => {
    if (attribute.attributeType === umlClass_1.AttributeType.Array) {
        ...
        if (baseAttributeType !== umlClass_1.AttributeType.Elementary) {
            ...
            referenceStorageSection = (0, exports.parseStorageSectionFromAttribute)(baseAttribute, umlClass, otherClasses, storageSections, mapping);
        }
        const displayValue = calcDisplayValue(baseAttribute.attributeType, dynamicBase, mapping, referenceStorageSection?.type);
        const getValue = calcGetValue(attribute.attributeType, mapping);
        const variables = [];
        variables[0] = {
            id: variableId++,
    ...
    if (attribute.attributeType === umlClass_1.AttributeType.UserDefined) {
        ...
        if (typeClass.stereotype === umlClass_1.ClassStereotype.Struct) {
            let variables = parseVariables(typeClass, otherClasses, [], storageSections, [], mapping);
            const newStorageSection = {
                id: storageId++,
                name: attribute.type,
    ...
    if (attribute.attributeType === umlClass_1.AttributeType.Mapping) {
        ...
        if (result !== null && result[1] && !(0, exports.isElementary)(result[1])) {
            ...
            if (typeClass.stereotype === umlClass_1.ClassStereotype.Struct) {
                let variables = parseVariables(typeClass, otherClasses, [], storageSections, [], true);
                const newStorageSection = {
                    id: storageId++,
                    name: typeClass.name,
                    type: StorageSectionType.Struct,
 ...
 */
const addDynamicVariables = async (storageSection, storageSections, url, contractAddress, blockTag) => {
    for (const variable of storageSection.variables) {
        ...
        if (variable.type === 'string' || variable.type === 'bytes') {
            ...
            if (size > 31) {
                ...
                await (0, slotValues_1.addSlotValues)(url, contractAddress, newStorageSection, blockTag);
                storageSections.push(newStorageSection);
            }
            continue;
        }
        ...
        await (0, exports.addDynamicVariables)(referenceStorageSection, storageSections, url, contractAddress, blockTag);
        if (!variable.dynamic)
            continue;
        ...
        for (let i = 1; i < arrayLength; i++) {
            ...
            await (0, slotValues_1.addSlotValues)(url, contractAddress, referenceStorageSection, blockTag);
        }
    }
};
